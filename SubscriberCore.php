   ...
   */
   function VerifyUniqueEmail($strEmail){
    if(!Database::Get()->GetUserDataByEmail($strEmail)){
      ...
      Debug::Debug_er('$strDomain ['.$strDomain.'] METHOD ['.__METHOD__.'] LINE['.__LINE__.']',1);
      if(fopen("https://www.$strDomain")){
      Debug::Debug_er('$strDomain SUCCESS ['.$strDomain.'] METHOD ['.__METHOD__.'] LINE['.__LINE__.']',1);

      }
      Debug::Debug_er('$strDomain ['.$strDomain.'] METHOD ['.__METHOD__.'] LINE['.__LINE__.']',1);
      $arrDomainCheck = Utility::Get()->GetURLHeaderHTTP('https://www.'.$strDomain,TRUE);
      $strDomainCheck = var_export($arrDomainCheck,TRUE);
      DisplayMessages::Get()->AddUserMSG( '$strDomainCheck ['.$strDomainCheck.'].',1);
      if(!Utility::Get()->GetURLHeaderHTTP($strDomain)){//check for a valid URL
